...
uint256 hashGenesisBlock("0x316432fc2fd2f999436288735f2c3befe3dced3d3430bcaa45edab0fe969780c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LocalFamilyOwned: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LocalFamilyOwned Signed Message:\n";
...
{
    // LocalFamilyOwned: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1){
        nSubsidy = nSubsidy * 40000000 * 10  / 100 / 24;
     }
...
static const int64 nTargetTimespan = 1234800;
static const int64 nTargetSpacing = 420;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LocalFamilyOwned: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LocalFamilyOwned: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LocalFamilyOwned: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x316432fc2fd2f999436288735f2c3befe3dced3d3430bcaa45edab0fe969780c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1424784551;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085323055;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LocalFamilyOwnedMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LocalFamilyOwnedMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running LocalFamilyOwned with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LocalFamilyOwnedMiner terminated\n");
        throw;
    }
}
